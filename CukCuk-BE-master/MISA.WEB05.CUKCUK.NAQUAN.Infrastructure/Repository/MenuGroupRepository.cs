...
namespace MISA.WEB05.CUKCUK.NAQUAN.Infrastructure.Repository
{
    ...
    {
        private IMenuGroupRepository _menuGroupRepositoryImplementation;
        ...
        public Guid InsertMenuGroup(MenuGroup MenuGroup)
        {
            ...
            using (var transaction = mySqlConnection.BeginTransaction())
            {
                try
                {
                    var procCommnand = $"Proc_Insert_MenuGroup";
                    var paramProc = new DynamicParameters(MenuGroup);
                    var newGuid = Guid.NewGuid();
                    MenuGroup.MenuGroupID = newGuid;
