...
namespace MISA.WEB05.CUKCUK.NAQUAN.Application.Services
{
    public class FoodService : BaseService<Food> , IFoodService
    {
        ...
        public FoodService(IFoodRepository FoodRepository, IConfiguration configuration ) : base(FoodRepository)
        {
        ...
        {
            // Định dạng những đuôi file được phép lưu
            var supportTypes = new[] { ".jpg",".JPG", ".jpeg", ".png", ".gif" };
            ...
            {
                throw new ErrorException(devMsg: "Vui lòng chọn ảnh có dung lượng nhỏ hơn 5 MB");
            }

            string fileName = Guid.NewGuid() + fileExt;

            // Tạo đường dẫn tới thư mục để lưu
            ...
            // Khởi tạo đường dẫn ảnh đến thư mục
        ...
        {
            return _FoodRepository.DeleteFood(ids);
        }
        ...
        public string GetNewCode(string name)
        {
            ...
            if (words.Length < 2)
            {
                ...
                if (_FoodRepository.IsDuplicate(Guid.Empty, foodCode) == false)
                {
                    return foodCode;
                }
                else
                {
                    return _FoodRepository.GetNewCodeNumber(foodCode);
                }
            }
            else
            {
        ...
        {
            string m_Paging; // chuỗi điều kiện phân trang
            string m_Where = "WHERE 1 = 1 "; // điều kiện lọc mặc định 1=1  
            string? m_Sort = null; // sắp xếp dữ liệu
            
            // nếu có filter thì thêm đk lọc
            ...
            var res = _FoodRepository.FilterFood(m_Where, m_Sort, m_Paging);
        ...
        public ImageDto GetImage(string imageName)
        {
            ...
            return new ImageDto { ImageBytes = imageBytes, MimeType = mimeType };
        }
        ...
        {
            return _FoodRepository.IsDuplicate(foodID, foodCode);
        }
