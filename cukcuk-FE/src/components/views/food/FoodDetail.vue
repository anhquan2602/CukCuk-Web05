<template>
  <div class="wrap-popup">
          ...
          <div class="popup-body">
              ...
              >
                <div class="popup__body-content-left">
                    ...
                    <div class="popup__input">
                      <div class="wrap-input-detail">
                        <BaseCombobox
                          ...
                          class="combobox"
                          name="menuGroupName"
                          url="https://localhost:7123/api/v1/MenuGroups"
                        ...
                        <div
                          ...
                          @click="
                            handleShowDialog('MenuGroups', 'Thêm nhóm thực đơn')
                          "
                        ></div>
                      </div>
                    </div>
                  ...
                  <div class="popup__group__input flex flex flex-items-center">
                    <label class="popup__group__input-label"
                      >Đơn vị tính
                      <span class="lable-required">(*)</span></label
                    >
                    <div class="popup__input">
                      <div class="wrap-input-detail">
                        <BaseCombobox
                          ...
                          class="combobox"
                          name="foodUnitName"
                          url="https://localhost:7123/api/v1/FoodUnits"
                        ...
                        <div
                          class="invisible-button"
                          @click="
                            handleShowDialog('FoodUnits', 'Thêm đơn vị tính')
                          "
                        ></div>
                      </div>
                    </div>
                  ...
                  >
                    ...
                    <div class="popup__input">
                      <input
                        ...
                        @input="inputNumber"
                        @keyup="
                          ($event) => updateValue($event, food, 'FoodCost')
                        "
                    ...
                    <div class="popup__input">
                      <div class="wrap-input-detail">
                        <BaseCombobox
                          ...
                          class="combobox"
                          name="cookRoomName"
                          url="https://localhost:7123/api/v1/CookRooms"
                        ...
                        <div
                          class="invisible-button"
                          @click="handleShowDialog('CookRooms', 'Thêm nơi nấu')"
                        ></div>
                      </div>
                    </div>
                  </div>
            ...
            >
              <div class="popup__body-footer-right flex flex-items-center">
                ...
                >
                  <div
                    class="btn-icon icon-toolbar icon-16 icon-form-save"
                  ></div>
    ...
    <FormAdd @handleReloadData="handleReloadData" />
  </div>
</template>
<script>
...
// import {getAllQuickAdd} from "@/utils/quickAdd/quickadd.js";
import {
  InsertFood,
  UpdateFood,
  UploadImage,
...
import {
  getAllService,
  getServiceByID,
} from "@/utils/servicehobbys/loadServiceHobby";
import {
  getNewCode,
  getFoodByID,
  checkDuplicateCode,
...
import FormAdd from "../addMore/FormAdd.vue";
...
export default {
  ...
  methods: {
    // Xử lí khi bấm nút thêm đơn vị
    handleShowDialog(title, headerName) {
    ...
    async handleReloadData(ref) {
      await this.$refs[`${ref}`].loadData();
    },

    //  kiểm tra có tên sở thích phục vụ chưa
    checkNoNameService() {
    ...
    /**
     * Hiện thị Menu service
     * author : NAQUAN (26/8/2023)
     * @param {*} event
     * @param {*} index
     */
    ...
    inputOnChange(value) {
      try {
        const lowercasedValue = value.toLowerCase(); // Chuyển giá trị tìm kiếm thành chữ thường

        this.filteredServiceHobbys = this.serviceHobbys.filter(
          (item) =>
            item.ServiceHobbyName.toLowerCase().includes(lowercasedValue) || // Chuyển giá trị trong dữ liệu thành chữ thường để so sánh
            item.AdditionalPayment.toString().includes(lowercasedValue)
        );

        this.showService = this.filteredServiceHobbys.length > 0; // Có kết quả thì hiển thị
      } catch (error) {
        console.log(error);
      }
    },
      ...
      try {
        if (
          ...
          if (event.target.files[0].size > 5 * 1024 * 1024) {
            ...
            _this.showPopupFunc(Enum.typePopup.errorImgOverSize);
            _this.msgPopup = Sources.popupMsg.errorImgOverSize;
          } else {
        ...
        if (
          ...
          if (res.status == 200) {
            ...
            if (this.food.FoodImageID) {
              ...
              this.urlFileImage = `https://localhost:7123/api/v1/Foods/get-image/${this.food.FoodImageID}`;
            }
    ...
    /**
     * Hàm lấy dữ liệu sở thích phục vụ
     * author: NA Quân(25/08/2023)
     */
    async getAllService() {
      let res;
      ...
      if (res.status == 200) {
        this.serviceHobbys = res.data;
      }
    },

    /**
     * Hàm xử lí cất dữ liệu
     * author: NA Quân(25/08/2023)
     */
    async handleStore() {
      let res;
      ...
      // Check trùng mã
      const checkCode = await this.checkDuplicateCodeFunc(
        this.guidNull,
        this.food.FoodCode
      );

      // Check Sở thích phục vụ trùng nhau
      const checkService = this.checkAddListService();

      // Check có thu thêm nhưng không có tên sở thích phục vụ
      const checkNoNameService = this.checkNoNameService();
      ...
      if (
        ...
        if (this.fileImage) {
          try {
            ...
            this.msgPopup = error.response.data.devMsg;
            this.$emit("showForm", true);
            return;
          }
        }
