<template>
  <div class="combobox">
    ...
    />
    <button
      class="button combobox__button icon__input--down icon-16 input-btn"
      @click.stop="btnSelectDataOnClick"
      @keydown="selecItemUpDown"
    ></button>
    <button class="button-add" @click="$emit('handleShowDialog')"></button>
    <!-- Cần thêm icon -->
    <!-- <button class="button plus__button"></button> -->
    <div
      v-if="isShowListData"
...
export default {
  ...
  // updated() {
  //   // Nếu thêm mới dữ liệu thì gọi lại api để lấy dữ liệu mới:
  //   if (this.url) {
  //     fetch(this.url)
  //       .then((res) => res.json())
  //       .then((res) => {
...
.button-add {
  ...
  border-left: unset;
  background-color: #fff;
  background: url(../../../assets/Resource/add-blue-icon.png) no-repeat center;
}
</style>